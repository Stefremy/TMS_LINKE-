...
async function run() {
  ...
  const clientShipments = allShipments.filter(s => s.client_id === 'b79cab49-629a-49f1-99c6-ef711e040cf7');
  console.log("Total for client:", clientShipments.length);
  ...
  const pendingShipments = clientShipments.filter(s => !invoicedIds.has(s.id));
  console.log("Pending for client:", pendingShipments.map(s => ({id: s.id.substring(0,8), status: s.status, price: s.sell_price})));
}
run();
