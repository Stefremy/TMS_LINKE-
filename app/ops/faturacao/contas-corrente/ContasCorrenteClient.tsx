...
import { Building, FileText, ChevronDown, ChevronRight, Euro, Cloud, AlertCircle, Filter } from "lucide-react"

export default function ContasCorrenteClient({ clients, shipments }: { clients: any[], shipments: any[] }) {
  ...
  return (
    <div className="bg-slate-50 min-h-[calc(100vh-8rem)]">
      ...
      <div className="bg-white border border-slate-200 shadow-2xs rounded-2xl overflow-hidden">
        ...
        <div className="divide-y divide-slate-100">
          {clientDataMap.length === 0 ? (
             <div className="px-6 py-12 text-center">
               ...
               <h3 className="text-sm font-bold text-slate-600">Nenhum dado encontrado</h3>
               <p className="text-xs text-slate-400 mt-1">Não existem clientes com envios nestas datas.</p>
             </div>
          ) : (
                      ...
                      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-2xs">
                        ...
                        <div className="divide-y divide-slate-50">
                          {shipments.map(s => {
                            ...
                            return (
                              <div key={s.id} className={`px-4 py-3 flex items-center justify-between hover:bg-slate-50 ${isExcluded ? "opacity-50" : ""}`}>
                                <div className="flex items-center gap-3">
                                  <input 
                                    type="checkbox" 
                                    checked={!isExcluded}
